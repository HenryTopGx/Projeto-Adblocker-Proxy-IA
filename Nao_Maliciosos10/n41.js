...
(function() {
    ...
    (function(y, v) {
        ...
        function h(p, w, z) {
            ...
            w && (A.fill = "both",
            A.duration = "auto");
            typeof p != "number" || isNaN(p) ? p !== void 0 && Object.getOwnPropertyNames(p).forEach(function(B) {
                if (p[B] != "auto") {
                    if (typeof A[B] == "number" || B == "duration")
                        if (typeof p[B] != "number" || isNaN(p[B]))
                            return;
                    B == "fill" && e.indexOf(p[B]) == -1 || B == "direction" && m.indexOf(p[B]) == -1 || B == "playbackRate" && p[B] !== 1 && y.isDeprecated("AnimationEffectTiming.playbackRate", "2014-11-28", "Use Animation.playbackRate instead.") || (A[B] = p[B])
                }
            }) : A.duration = p;
            return A
        }
        function d(p, w, z, A) {
            return p < 0 || p > 1 || z < 0 || z > 1 ? n : function(B) {
                if (B <= 0) {
                    var D = 0;
                    p > 0 ? D = w / p : !w && z > 0 && (D = A / z);
                    return D * B
                }
                if (B >= 1)
                    return D = 0,
                    z < 1 ? D = (A - 1) / (z - 1) : z == 1 && p < 1 && (D = (w - 1) / (p - 1)),
                    1 + D * (B - 1);
                D = 0;
                for (var H = 1; D < H; ) {
                    var E = (D + H) / 2
                      , M = 3 * p * (1 - E) * (1 - E) * E + 3 * z * (1 - E) * E * E + E * E * E;
                    if (Math.abs(B - M) < 1E-5)
                        break;
                    M < B ? D = E : H = E
                }
        ...
        function c(p) {
            ...
            if (w == "" && f())
                throw new TypeError(p + " is not a valid value for easing");
            return w
        }
        function b(p) {
            if (p == "linear")
        ...
        k.prototype = {
            _setMember: function(p, w) {
                this["_" + p] = w;
                this._effect && (this._effect._timingInput[p] = w,
                this._effect._timing = y.normalizeTimingInput(this._effect._timingInput),
                this._effect.activeDuration = y.calculateActiveDuration(this._effect._timing),
            ...
            set iterationStart(p) {
                if ((isNaN(p) || p < 0) && f())
                    throw new TypeError("iterationStart must be a non-negative number, received: " + p);
            ...
            set duration(p) {
                if (p != "auto" && (isNaN(p) || p < 0) && f())
                    throw new TypeError("duration must be non-negative or auto, received: " + p);
            ...
            set iterations(p) {
                if ((isNaN(p) || p < 0) && f())
                    throw new TypeError("iterations must be non-negative, received: " + p);
        ...
        y.calculateActiveDuration = function(p) {
            var w = Math
              , z = w.abs;
            var A = p.duration === 0 || p.iterations === 0 ? 0 : p.duration * p.iterations;
        ...
        y.calculateIterationProgress = function(p, w, z) {
            ...
            a: {
                ...
                switch (A) {
                case 1:
                    p = B == "backwards" || B == "both" ? 0 : null;
                    break a;
                case 3:
                    p = w - z.delay;
                    break a;
                case 2:
                    p = B == "forwards" || B == "both" ? p : null;
                    break a;
                case 0:
                    p = null;
                    break a
            ...
            w = z.duration;
            B = z.iterationStart;
            w === 0 ? A !== 1 && (B += z.iterations) : B += p / w;
            w = B;
            B = w === Infinity ? z.iterationStart % 1 : w % 1;
            B !== 0 || A !== 2 || z.iterations === 0 || p === 0 && z.duration !== 0 || (B = 1);
            p = B;
            B = A === 2 && z.iterations === Infinity ? Infinity : p === 1 ? Math.floor(w) - 1 : Math.floor(w);
            w = A = z.direction;
            A !== "normal" && A !== "reverse" && (A === "alternate-reverse" && (B += 1),
            w = "normal",
            B !== Infinity && B % 2 !== 0 && (w = "reverse"));
            return z._easingFunction(w === "normal" ? p : 1 - p)
        }
        ;
        y.calculatePhase = g;
        y.normalizeEasing = c;
    ...
    (function(y, v) {
        function k(a) {
            var c = [], b;
            for (b in a)
                if (!(b in ["easing", "offset", "composite"])) {
                    var g = a[b];
                    Array.isArray(g) || (g = [g]);
                    for (var e, m = g.length, n = 0; n < m; n++)
                        e = {},
                        e.offset = "offset"in a ? a.offset : m == 1 ? 1 : n / (m - 1),
        ...
        y.normalizeKeyframes = function(a) {
            function c() {
                var n = b.length;
                b[n - 1].offset == null && (b[n - 1].offset = 1);
                n > 1 && b[0].offset == null && (b[0].offset = 0);
                for (var l = 0, r = b[0].offset, t = 1; t < n; t++) {
                    var x = b[t].offset;
                    if (x != null) {
                        for (var u = 1; u < t - l; u++)
                            b[l + u].offset = r + (x - r) * u / (t - l);
                        l = t;
                        r = x
                    }
                }
            ...
            var b = a.map(function(n) {
                ...
                for (r in n) {
                    ...
                    if (r == "offset") {
                        ...
                        if (t != "replace")
                            throw new TypeError("Invalid composite mode " + t + ".");
                    } else
                        t = r == "easing" ? y.normalizeEasing(t) : "" + t;
                    var x = void 0
                      , u = r
                      , q = t;
                    t = l;
                    var C = u;
                    if (C !== "display" && C.lastIndexOf("animation", 0) !== 0 && C.lastIndexOf("transition", 0) !== 0)
                        if (C = f[u])
                            for (x in h.style[u] = q,
                            C) {
                                q = u = C[x];
                                var p = h.style[u];
                                t[u] = q in d ? d[q][p] || p : p
                            }
                        else
                            t[u] = u in d ? d[u][q] || q : q
                }
                l.offset == void 0 && (l.offset = null);
                l.easing == void 0 && (l.easing = "linear");
                return l
            });
            a = !0;
    ...
    (function(y) {
        ...
        y.isDeprecated = function(k, f, h, d) {
            ...
            return a < f ? (k in v || console.warn("Web Animations: " + k + " " + d + " deprecated and will stop working on " + f.toDateString() + ". " + h),
            v[k] = !0,
            !1) : !0
        }
        ;
        y.deprecated = function(k, f, h, d) {
            var a = d ? "are" : "is";
            if (y.isDeprecated(k, f, h, d))
                throw Error(k + " " + a + " no longer supported. " + h);
        }
    }
    )(G);
    (function() {
        if (document.documentElement.animate) {
            var y = document.documentElement.animate([], 0)
              , v = !0;
            y && (v = !1,
            "play currentTime pause reverse playbackRate cancel finish startTime playState".split(" ").forEach(function(k) {
                y[k] === void 0 && (v = !0)
            }));
            if (!v)
                return
        ...
        (function(k, f, h) {
            function d(c) {
                for (var b = {}, g = 0; g < c.length; g++)
                    for (var e in c[g])
                        if (e != "offset" && e != "easing" && e != "composite") {
                            var m = {
                                offset: c[g].offset,
                                easing: c[g].easing,
                                value: c[g][e]
                ...
                for (var n in b)
                    if (c = b[n],
                    c[0].offset != 0 || c[c.length - 1].offset != 1)
                        throw {
                            type: DOMException.NOT_SUPPORTED_ERR,
                            name: "NotSupportedError",
            ...
            function a(c) {
                var b = [], g;
                for (g in c)
                    for (var e = c[g], m = 0; m < e.length - 1; m++) {
                        var n = m
                          , l = m + 1
                          , r = e[n].offset
                          , t = e[l].offset
                ...
                return function(e, m) {
                    if (m != null)
                        g.filter(function(l) {
                            ...
                            r = t == 0 ? 0 : l.easingFunction(r / t);
                            f.apply(e, l.property, l.interpolation(r))
                        });
                    else
                        for (var n in b)
                            n != "offset" && n != "easing" && n != "composite" && f.clear(e, n)
                }
            }
        }
        )(G, F, null);
        (function(k, f, h) {
            ...
            f.propertyInterpolation = function(b, g, e) {
                ...
                for (m = 0; b && m < b.length; m++) {
                    var n = b[m][0](g)
                      , l = b[m][0](e);
                    if (n !== void 0 && l !== void 0 && (n = b[m][1](n, l))) {
                        var r = f.Interpolation.apply(null, n);
                        return function(t) {
                            return t == 0 ? g : t == 1 ? e : r(t)
                        }
                    }
                }
        ...
        (function(k, f, h) {
            ...
            f.KeyframeEffect = function(a, c, b, g) {
                var e = d(k.normalizeTimingInput(b)), m = f.convertEffectInput(c), n;
                c = function() {
                    m(a, n)
                }
                ;
        ...
        (function(k, f) {
            function h(d, a, c) {
                if (typeof d == "number" && typeof a == "number")
                    return d * (1 - c) + a * c;
                if (typeof d == "boolean" && typeof a == "boolean")
                    return c < .5 ? d : a;
        ...
        (function(k, f, h) {
            ...
            f.Animation.prototype = {
                _ensureAlive: function() {
                    this._inEffect = this.playbackRate < 0 && this.currentTime === 0 ? this._effect._update(-1) : this._effect._update(this.currentTime);
                    this._inTimeline || !this._inEffect && this._finishedFlag || (this._inTimeline = !0,
                    f.timeline._animations.push(this))
                },
                ...
                set currentTime(a) {
                    a = +a;
                    isNaN(a) || (f.restart(),
                    this._paused || this._startTime == null || (this._startTime = this._timeline.currentTime - a / this._playbackRate),
                    this._currentTimePending = !1,
                ...
                set playbackRate(a) {
                    if (a != this._playbackRate) {
                        ...
                        this._startTime = null;
                        this.playState != "paused" && this.playState != "idle" && (this._idle = this._finishedFlag = !1,
                        this._ensureAlive(),
                        f.applyDirtiedAnimation(this));
                        c != null && (this.currentTime = c)
                    }
                },
                get _isFinished() {
                ...
                addEventListener: function(a, c) {
                    typeof c == "function" && a == "finish" && this._finishHandlers.push(c)
                },
                removeEventListener: function(a, c) {
                    a == "finish" && (a = this._finishHandlers.indexOf(c),
                    a >= 0 && this._finishHandlers.splice(a, 1))
                },
                ...
                _tick: function(a, c) {
                    this._idle || this._paused || (this._startTime == null ? c && (this.startTime = a - this._currentTime / this.playbackRate) : this._isFinished || this._tickCurrentTime((a - this._startTime) * this.playbackRate));
        ...
        (function(k, f, h) {
            ...
            function g(q, C, p) {
                ...
                p.forEach(function(D) {
                    D._tick(q, C);
                    D._inEffect ? (z.push(D._effect),
                    D._markTarget()) : (w.push(D._effect),
                    D._unmarkTarget());
                    D._needsTick && (l = !0);
            ...
            f.applyDirtiedAnimation = function(q) {
                x || (q._markTarget(),
                q = q._targetAnimations(),
                q.sort(a),
                g(f.timeline.currentTime, !1, q.slice())[1].forEach(function(C) {
                    C = u._animations.indexOf(C);
                    C !== -1 && u._animations.splice(C, 1)
                }),
                b())
            }
            ;
        ...
        (function(k) {
            function f(c, b) {
                var g = c.exec(b);
                if (g)
                    return g = c.ignoreCase ? g[0].toLowerCase() : g[0],
                    [g, b.substr(g.length)]
            }
            function h(c, b) {
                b = b.replace(/^\s*/, "");
            ...
            k.consumeParenthesised = function(c, b) {
                for (var g = 0, e = 0; e < b.length && (!/\s|,/.test(b[e]) || g != 0); e++)
                    if (b[e] == "(")
                        g++;
                    else if (b[e] == ")" && (g--,
                    g == 0 && e++,
                    g <= 0))
                        break;
                c = c(b.substr(0, e));
                return c == void 0 ? void 0 : [c, b.substr(e)]
            }
            ;
            k.ignore = function(c) {
                return function(b) {
            ...
            k.mergeList = function(c, b, g) {
                for (var e = [], m = [], n = [], l = 0, r = 0; r < g.length; r++)
        ...
        (function(k) {
            function f(d) {
                ...
                if ((d = k.consumeRepeated(function(c) {
                    ...
                    if (b)
                        return a.inset = !0,
                        b;
                    if (b = k.consumeLengthOrPercent(c))
                        return a.lengths.push(b[0]),
                        b;
                    if (b = k.consumeColor(c))
                        return a.color = b[0],
                        b
                }, /^/, d)) && d[0].length)
                    return [a, d[1]]
            ...
            .bind(null, function(d, a) {
                for (; d.lengths.length < Math.max(d.lengths.length, a.lengths.length); )
                    d.lengths.push({
                        px: 0
                    });
                for (; a.lengths.length < Math.max(d.lengths.length, a.lengths.length); )
                    a.lengths.push({
                        px: 0
                    });
                if (d.inset == a.inset && !!d.color == !!a.color) {
                    for (var c = [], b, g = [[], 0], e = [[], 0], m = 0; m < d.lengths.length; m++) {
                        var n = k.mergeDimensions(d.lengths[m], a.lengths[m], m == 2);
                        g[0].push(n[0]);
                        e[0].push(n[1]);
                        c.push(n[2])
                    }
                    d.color && a.color && (a = k.mergeColors(d.color, a.color),
                    g[1] = a[0],
                    e[1] = a[1],
                    b = a[2]);
                    return [g, e, function(l) {
                        for (var r = d.inset ? "inset " : " ", t = 0; t < c.length; t++)
                            r += c[t](l[0][t]) + " ";
                        b && (r += b(l[1]));
                        return r
                    }
            ...
            k.addPropertiesHandler(function(d) {
                if ((d = k.consumeRepeated(f, /^,/, d)) && d[1] == "")
                    return d[0]
        ...
        (function(k, f) {
            k.addPropertiesHandler(String, function(h, d) {
                if (h == "visible" || d == "visible")
                    return [0, 1, function(a) {
                        return a <= 0 ? h : a >= 1 ? d : "visible"
                    }
                    ]
            }, ["visibility"])
        }
        ...
        (function(k, f) {
            function h(c) {
                ...
                a.fillStyle = c;
                if (b == a.fillStyle)
                    return a.fillRect(0, 0, 1, 1),
                    c = a.getImageData(0, 0, 1, 1).data,
                    a.clearRect(0, 0, 1, 1),
                    b = c[3] / 255,
                    [c[0] * b, c[1] * b, c[2] * b, b]
            }
            function d(c, b) {
                return [c, b, function(g) {
                    if (g[3])
                        for (var e = 0; e < 3; e++)
                            g[e] = Math.round(Math.max(0, Math.min(255, g[e] / g[3])));
        ...
        (function(k, f) {
            ...
            function d(m, n) {
                ...
                if (/^[^(]*$|^calc/.test(n)) {
                    ...
                    m = "U(" + m.source + ")";
                    for (var r = n.replace(/[-+]?(\d*\.)?\d+([Ee][-+]?\d+)?/g, "N").replace(new RegExp("N" + m,"g"), "D").replace(/\s[+-]\s/g, "O").replace(/\s/g, ""), t = [/N\*(D)/g, /(N|D)[*/]N/g, /(N|D)O\1/g, /\((N|D)\)/g], x = 0; x < t.length; )
                        t[x].test(r) ? (r = r.replace(t[x], "$1"),
                        x = 0) : x++;
                    if (r == "D") {
                        for (var u in l) {
            ...
            function c(m, n, l) {
                ...
                return [m, n, function(x) {
                    var u = x.map(function(q, C) {
                        x.length == 1 && l && (q = Math.max(q, 0));
                        return k.numberToString(q) + r[C]
                    }).join(" + ");
                    return x.length > 1 ? "calc(" + u + ")" : u
                }
                ]
            }
        ...
        (function(k, f) {
            ...
            function d(a) {
                if ((a = k.consumeList([k.ignore(k.consumeToken.bind(null, /^rect/)), k.ignore(k.consumeToken.bind(null, /^\(/)), k.consumeRepeated.bind(null, h, /^,/), k.ignore(k.consumeToken.bind(null, /^\)/))], a)) && a[0].length == 4)
                    return a[0]
            }
            f = k.mergeWrappedNestedRepeated.bind(null, function(a) {
                return "rect(" + a + ")"
            }, function(a, c) {
                return a == "auto" || c == "auto" ? [!0, !1, function(b) {
                    b = b ? a : c;
                    if (b == "auto")
                        return "auto";
        ...
        (function(k, f) {
            ...
            function a(l) {
                ...
                for (var r = /\s*(\w+)\(([^)]*)\)/g, t = [], x, u = 0; (x = r.exec(l)) && x.index == u; ) {
                    ...
                    for (var p = [], w = 0; w < C.length; w++) {
                        ...
                        z = z ? {
                            A: function(B) {
                                return B.trim() == "0" ? m : k.parseAngle(B)
                            },
                            N: k.parseNumber,
            ...
            function b(l, r) {
                ...
                if (t[0] == null || x[0] == null)
                    return [[!1], [!0], function(u) {
            ...
            k.addPropertiesHandler(a, function(l, r) {
                ...
                if (l.length != r.length) {
                    ...
                    for (u = 0; u < l.length; u++) {
                        ...
                        A = n[q];
                        var E = n[B];
                        if (q == "perspective" && B == "perspective" || !(q != "matrix" && q != "matrix3d" || B != "matrix" && B != "matrix3d")) {
                            if (!t)
                                return;
                            A = b([l[u]], [r[u]]);
                            C.push(A[0]);
                            ...
                            if (q != B)
                                if (A[2] && E[2] && q.replace(/[xy]/, "") == B.replace(/[xy]/, ""))
                                    q = q.replace(/[xy]/, ""),
                                    D = A[2](D),
                                    H = E[2](H);
                                else if (A[1] && E[1] && g(q) == g(B))
                                    q = g(q),
                                    D = A[1](D),
                                    H = E[1](H);
                                else {
                            ...
                            for (var M = [], K = 0; K < D.length; K++)
                                A = (typeof D[K] == "number" ? k.mergeNumbers : k.mergeDimensions)(D[K], H[K]),
                                E[K] = A[0],
                                B[K] = A[1],
                                M.push(A[2]);
                            C.push(E);
                ...
                return [C, w, function(N) {
                    return N.map(function(I, J) {
                        ...
                        z[J][0] == "matrix" && I.split(",").length == 16 && (z[J][0] = "matrix3d");
                        return z[J][0] + "(" + I + ")"
                    }).join(" ")
                }
                ]
        ...
        if (document.createElement("div").animate([]).oncancel === void 0) {
            ...
            window.Element.prototype.animate = function(f, h) {
                ...
                f.addEventListener = function(b, g) {
                    typeof g == "function" && b == "cancel" ? this._cancelHandlers.push(g) : a.call(this, b, g)
                }
                ;
                var c = f.removeEventListener;
                f.removeEventListener = function(b, g) {
                    b == "cancel" ? (b = this._cancelHandlers.indexOf(g),
                    b >= 0 && this._cancelHandlers.splice(b, 1)) : c.call(this, b, g)
                }
                ;
                return f
            }
    ...
    (function(y) {
        var v = document.documentElement
          ...
          , f = !1;
        try {
            var h = getComputedStyle(v).getPropertyValue("opacity") == "0" ? "1" : "0";
            k = v.animate({
                opacity: [h, h]
            }, {
                duration: 1
        ...
        if (!f) {
            ...
            window.Element.prototype.animate = function(a, c) {
                window.Symbol && Symbol.iterator && Array.prototype.from && a[Symbol.iterator] && (a = Array.from(a));
                Array.isArray(a) || a === null || (a = y.convertToArrayForm(a));
    ...
    (function(y, v, k) {
        function f(c) {
            ...
            b._animations.length == 0 ? d = !1 : requestAnimationFrame(f)
        }
        ...
        v.AnimationTimeline.prototype = {
            ...
            _discardAnimations: function() {
                ...
                this._animations = this._animations.filter(function(c) {
                    return c.playState != "finished" && c.playState != "idle"
                })
            },
            _play: function(c) {
        ...
        v.Animation.prototype = {
            _updatePromises: function() {
                var f = this._oldPlayState
                  , h = this.playState;
                this._readyPromise && h !== f && (h == "idle" ? (this._rejectReadyPromise(),
                this._readyPromise = void 0) : f == "pending" ? this._resolveReadyPromise() : h == "pending" && (this._readyPromise = void 0));
                this._finishedPromise && h !== f && (h == "idle" ? (this._rejectFinishedPromise(),
                this._finishedPromise = void 0) : h == "finished" ? this._resolveFinishedPromise() : f == "finished" && (this._finishedPromise = void 0));
            ...
            _rebuildUnderlyingAnimation: function() {
                ...
                if (!this.effect || this.effect instanceof window.KeyframeEffect)
                    this._animation = v.newUnderlyingAnimationForKeyframeEffect(this.effect),
                    v.bindAnimationForKeyframeEffect(this);
                if (this.effect instanceof window.SequenceEffect || this.effect instanceof window.GroupEffect)
                    this._animation = v.newUnderlyingAnimationForGroup(this.effect),
                    v.bindAnimationForGroup(this);
                this.effect && this.effect._onsample && v.bindAnimationForCustomEffect(this);
                f && (h != 1 && (this.playbackRate = h),
                a !== null ? this.startTime = a : c !== null ? this.currentTime = c : this._holdTime !== null && (this.currentTime = this._holdTime),
                d && this.pause());
                this._updatePromises()
            },
            ...
            _setExternalAnimation: function(f) {
                if (this.effect && this._isGroup)
                    for (var h = 0; h < this.effect.children.length; h++)
                        this.effect.children[h]._animation = f,
                        this._childAnimations[h]._setExternalAnimation(f)
            },
            ...
            _arrangeChildren: function(f, h) {
                this.startTime === null ? f.currentTime = this.currentTime - h / this.playbackRate : f.startTime !== this.startTime + h / this.playbackRate && (f.startTime = this.startTime + h / this.playbackRate)
            },
            get timeline() {
            ...
            get finished() {
                if (!window.Promise)
                    return console.warn("Animation Promises require JavaScript Promise constructor"),
                    null;
                this._finishedPromise || (v.animationsWithPromises.indexOf(this) == -1 && v.animationsWithPromises.push(this),
                this._finishedPromise = new Promise(function(f, h) {
                ...
                .bind(this)),
                this.playState == "finished" && this._resolveFinishedPromise());
            ...
            get ready() {
                if (!window.Promise)
                    return console.warn("Animation Promises require JavaScript Promise constructor"),
                    null;
                this._readyPromise || (v.animationsWithPromises.indexOf(this) == -1 && v.animationsWithPromises.push(this),
                this._readyPromise = new Promise(function(f, h) {
                ...
                .bind(this)),
                this.playState !== "pending" && this._resolveReadyPromise());
            ...
            set onfinish(f) {
                this._animation.onfinish = typeof f == "function" ? function(h) {
                    h.target = this;
                    f.call(this, h)
                }
                .bind(this) : f
            ...
            set oncancel(f) {
                this._animation.oncancel = typeof f == "function" ? function(h) {
                    h.target = this;
                    f.call(this, h)
                }
                .bind(this) : f
            ...
            reverse: function() {
                ...
                f !== null && (this.currentTime = f);
                this._updatePromises()
            },
            addEventListener: function(f, h) {
                var d = h;
                typeof h == "function" && (d = function(a) {
                    a.target = this;
                    h.call(this, a)
                }
                .bind(this),
            ...
            _forEachChild: function(f) {
                ...
                if (this.playState != "pending") {
                    var d = this.effect._timing
                      , a = this.currentTime;
                    a !== null && (a = y.calculateIterationProgress(y.calculateActiveDuration(d), a, d));
                    (a == null || isNaN(a)) && this._removeChildAnimations()
                }
            }
        };
    ...
    (function(y, v, k) {
        ...
        function h() {
            for (var e = !1; b.length; )
                b.shift()._updateChildren(),
                e = !0;
            return e
        }
        var d = function(e) {
            e._animation = void 0;
            if (e instanceof window.SequenceEffect || e instanceof window.GroupEffect)
        ...
        v.removeMulti = function(e) {
            for (var m = [], n = 0; n < e.length; n++) {
                var l = e[n];
                l._parent ? (m.indexOf(l._parent) == -1 && m.push(l._parent),
                l._parent.children.splice(l._parent.children.indexOf(l), 1),
                l._parent = null,
                d(l)) : l._animation && l._animation.effect == l && (l._animation.cancel(),
                l._animation.effect = new KeyframeEffect(null,[]),
                l._animation._callback && (l._animation._callback._animation = null),
                l._animation._rebuildUnderlyingAnimation(),
                d(l))
            }
        ...
        v.KeyframeEffect = function(e, m, n, l) {
            ...
            typeof m == "function" ? (y.deprecated("Custom KeyframeEffect", "2015-06-22", "Use KeyframeEffect.onsample instead."),
            this._normalizedKeyframes = m) : this._normalizedKeyframes = new f(m);
            this._keyframes = m;
        ...
        v.KeyframeEffect.prototype = {
            getFrames: function() {
                return typeof this._normalizedKeyframes == "function" ? this._normalizedKeyframes : this._normalizedKeyframes._frames
            },
            set onsample(e) {
        ...
        v.bindAnimationForKeyframeEffect = function(e) {
            e.effect && typeof e.effect._normalizedKeyframes == "function" && v.bindAnimationForCustomEffect(e)
        }
        ;
        var b = [];
        v.awaitStartTime = function(e) {
            e.startTime === null && e._isGroup && (b.length == 0 && requestAnimationFrame(h),
            b.push(e))
        }
        ;
        ...
        window.Element.prototype.getAnimations = function() {
            return document.timeline.getAnimations().filter(function(e) {
                return e.effect !== null && e.effect.target == this
            }
            .bind(this))
        }
    }
    ...
    (function(y, v, k) {
        ...
        function h(b) {
            ...
            b = b.filter(function(g) {
                ...
                e != "running" && e != "pending" && (g._registered = !1);
                return g._registered
            });
            a.push.apply(a, b);
            a.length ? (c = !0,
            requestAnimationFrame(h)) : c = !1
        }
        document.createElementNS("http://www.w3.org/1999/xhtml", "div");
        var d = 0;
        v.bindAnimationForCustomEffect = function(b) {
            var g = b.effect.target
              , e = typeof b.effect.getFrames() == "function";
            ...
            var r = function() {
                ...
                t !== null && (t = y.calculateIterationProgress(y.calculateActiveDuration(n), t, n),
                isNaN(t) && (t = null));
    ...
    (function(y, v, k) {
        ...
        function h(d, a, c) {
            ...
            this.timing._effect = this;
            this._timing.duration === "auto" && (this._timing.duration = this.activeDuration)
        }
        ...
        h.prototype = {
            ...
            _rebuild: function() {
                for (var d = this; d; )
                    d.timing.duration === "auto" && (d._timing.duration = d.activeDuration),
                    d = d._parent;
                this._animation && this._animation._rebuildUnderlyingAnimation()
            },
            ...
            _putChild: function(d, a) {
                for (var c = a ? "Cannot append an ancestor or self" : "Cannot prepend an ancestor or self", b = 0; b < d.length; b++)
                    if (this._isAncestor(d[b]))
                        throw {
                ...
                for (b = 0; b < d.length; b++)
                    a ? this.children.push(d[b]) : this.children.unshift(d[b]);
                this._reparent(d);
                this._rebuild()
            },
            append: function() {
            ...
            clone: function() {
                for (var d = y.cloneTimingInput(this._timingInput), a = [], c = 0; c < this.children.length; c++)
                    a.push(this.children[c].clone());
        ...
        v.newUnderlyingAnimationForGroup = function(d) {
            ...
            d.onsample = function(b) {
                var g = a._wrapper;
                g && g.playState != "pending" && g.effect && (b == null ? g._removeChildAnimations() : b == 0 && g.playbackRate < 0 && (c || (c = y.normalizeTimingInput(g.effect.timing)),
                b = y.calculateIterationProgress(y.calculateActiveDuration(c), -1, c),
                isNaN(b) || b == null) && (g._forEachChild(function(e) {
                    e.currentTime = -1
                }),
